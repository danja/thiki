...
public class WikiPage {
	...
	private static final String THIKI_PLACEHOLDER = "<_thiki.ph_>";
	...
	private String cloakTags(String html) {
		...
		while (m.find()) {
			...
			m.appendReplacement(sb, THIKI_PLACEHOLDER);
		}
		m.appendTail(sb);
		return sb.toString();
	}
	...
	private String uncloakTags(String html) {
		...
		while (m.find()) {
			...
			if (tag.equals(THIKI_PLACEHOLDER)) {
	...
	public String getHtmlBody() throws IOException {
		...
		while (m.find()) {
			...
			if (nowikiword == null) {
				...
				replacement = "<a href='thiki:" + replacement + "'>"
						+ replacement + "</a>";
			}
		...
		while (m.find()) {
			...
			String replacement = "<div class='thiki-task'>";
			...
			if (checked.equalsIgnoreCase("x")) {
				replacement += labelTagStart
						+ " class=thiki-task-finished'><input type='checkbox' checked='checked' "
						+ js + "/>";
			} else {
		...
		return "<html>"
				...
				+ "    <title>Thiki Wiki</title>"
				+ "    <style type='text/css'>"
				+ css
				+ "</style>"
				+ "    <script type='text/javascript'>"
				+ "      function toggleCheck(which) {"
				+ "        window.ThikiInterop.sendMessage('checkbox', which);"
				+ "        var lbl = document.getElementById('label_' + which); "
				+ "        if (lbl.className == 'thiki-task-finished') "
				+ "          lbl.className =  '';"
				+ "        else "
				+ "          lbl.className = 'thiki-task-finished';"
				+ "      }"
				+ "    </script>"
				+ "  </head>"
				+ "  <body>"
